...
public final class PlaceInvariantEquation {
  ...
  /**
   * Returns a formatted string representation of the equation, optionally using custom place names.
   *
   * <p>For example, if {@code placeNames} is {@code {"P0", "P1", "Buffer"}}, the output might be:
   * {@code 2*M(P0) + M(Buffer) = 12}
   *
   * <p>If {@code placeNames} is {@code null} or a place index is out of bounds, default names like
   * {@code p0}, {@code p1}, etc. are used.
   *
   * @param placeNames an array of custom names for places; may be {@code null}
   * @return the formatted equation string
   */
  public String formatEquation(String[] placeNames) {
    if (coefficients.isEmpty()) {
      return "0 = " + result;
    }
    ...
    for (Map.Entry<Integer, Integer> e : coefficients.entrySet()) {
      ...
      int place = e.getKey();
      String name =
          (placeNames != null && place < placeNames.length) ? placeNames[place] : ("p" + place);
